...
class Attachment extends Model
{
    ...
    //临时图片上传
    public function ls_upload($name, $user_id = 0,$path = '', $validate = [], $admin_user_id = 0)
    {
        ...
        $file = request()->withFiles([$name => $_FILES[$name]])->file($name);
        if ($file) {
            $file_url  = "/uploads/temporary/";
            $validate  = array_merge(config('attachment.validate'), $validate);
            $info      = $file->validate($validate)->move('./uploads/temporary/',$user_id);
            if ($info) {
                return str_replace("\\", '/', $file_url . $info->getSaveName());
            }
