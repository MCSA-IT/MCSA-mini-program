...
class IndexController extends Controller
{
    ...
    public function news()
    {
        ...
        if($this->request->post()){
            ...
            if($this->param['page']>$total||$this->param['page']==1){
                return $this->error();
            }
            $pageParam['page'] = $this->param['page'];
            $list = $model->where(['article_category_id'=>$this->cata_id])->order(['is_top'=>'desc','sort_number'=>'asc','id'=>'desc'])->append(['new'])->paginate($limit, false, $pageParam);
            return $this->success(1,'',$list);
        }
        ...
        $list = $model->where(['article_category_id'=>$this->cata_id])->order(['is_top'=>'desc','sort_number'=>'asc','id'=>'desc'])->append(['new'])->paginate($limit);
        $this->assign([
            'list'      => $list,
            'total'      => $total,
    ...
    public function search()
    {
        ...
        if($this->request->post()){
            ...
            $pageParam['page'] = $this->param['page'];
            $list = $model->where($map)->order(['is_top'=>'desc','sort_number'=>'asc','id'=>'desc'])->append(['new'])->paginate($limit, false, $pageParam);
            return $this->success(1,'',$list);
        }
