...
$(function () {
    $('body').on('click', '.AjaxButtonnew', function (event) {
        event.preventDefault();
...
/** 导出excel **/
function exportData_new(url) {
    var id_arr = $('input[name="data-checkbox"]:checked').map(function () {
        return this.value;
    }).get().join(",");
    if(id_arr==''){
        layer.msg('请选择导出的数据', {
    ...
    var exportUrl = url || 'index.html';
    var openUrl = exportUrl + '?export_data=1&' + $("#searchForm").serialize() + '&id='+id_arr;
    window.open(openUrl);
}

/** 导出excel **/
function exportData_two(url) {
    var exportUrl = url || 'index.html';
    var openUrl = exportUrl + '?export_data=2';
    window.open(openUrl);
}
